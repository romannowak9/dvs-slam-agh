...
def main() -> None:
    ...
    while processed < args.num_frames:
        ...
        if args.display:
            ...
            save_image(cv2, output_path, compare)
...
class CompressedImageStream:
    ...
    def _advance(self) -> None:
        ...
        raw_image = decode_compressed_image(cv2, msg)
        image = visualize_sensor_image(cv2, raw_image, self.image_kind)
...
def decode_compressed_image(cv2, msg) -> np.ndarray:
...
def visualize_sensor_image(cv2, image: np.ndarray, image_kind: str) -> np.ndarray:
    if image_kind == "depth":
        return visualize_depth(cv2, image)

    if image_kind in ("color", "infra"):
        return to_bgr(cv2, normalize_to_uint8(image))

    raise ValueError(f"Unsupported image kind: {image_kind}")


def visualize_depth(cv2, image: np.ndarray) -> np.ndarray:
...
def to_bgr(cv2, image: np.ndarray) -> np.ndarray:
    if image.ndim == 2:
...
def make_full_layout(
    ...
    color_image = prepare_tile(cv2, color_image, image_shape)
    depth_image = prepare_tile(cv2, depth_image, image_shape)
    event_image = prepare_tile(cv2, event_image, image_shape)
    infra_image = prepare_tile(cv2, infra_image, image_shape)

    draw_label(cv2, color_image, f"color  t={color_time:.6f}")
    draw_label(cv2, depth_image, f"depth  dt={format_dt(depth_time, color_time)}")
    draw_label(cv2, event_image, f"left events  dt={format_dt(event_time, color_time)}")
    draw_label(cv2, infra_image, f"infra  dt={format_dt(infra_time, color_time)}")
...
def draw_label(cv2, image: np.ndarray, text: str) -> None:
